...
/**
 * Ask for a new name for the session the route is showing.
 *
 * Every pure rule this dialog applies — the name check and the requirement sentence — comes
 * from `src/lib/utils/session-copy.ts`, which is the whole reason the project overview can
 * keep its own copy of this dialog without the two wordings ever drifting apart.
 */
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-1.5">
        <label htmlFor="rename-session-name" className="text-[13px] font-medium text-body-strong">
          Session name
        </label>
        <Input
          ...
          aria-describedby={isValid ? undefined : "rename-session-hint"}
          onChange={(event) => setValue(event.target.value)}
        />
        {!isValid && (
          <p id="rename-session-hint" className="text-xs text-muted">
            {SESSION_NAME_REQUIREMENT}
          </p>
        )}
      </div>
