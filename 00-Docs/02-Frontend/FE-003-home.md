...
## Mở rộng giai đoạn 18 — Quick Note nhúng Home

Contract FE-020 tại `FE-020-quick-note.md` hoàn thiện slot đã có: `src/app/home-entry.tsx` inject public `QuickNoteComposer` trước Notes sections, truyền suspended/live guard Data và Quit; feature Home không import Notes. Save thủ công/Cancel, feedback và draft do NotesProvider sở hữu; không thay Welcome/presence, Sessions hoặc Projects. Phần này supersede giới hạn “FE-019 chỉ inject Notes” khi FE-020 được triển khai. Cửa sổ nổi và shortcut/tray vẫn thuộc stage19; evidence lịch sử ở trên giữ nguyên.
