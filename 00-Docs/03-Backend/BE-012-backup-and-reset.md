...
### Quyết định tích hợp giai đoạn 13

- Giai đoạn 13 chỉ tạo parser/envelope v1 và participant Projects, Settings, CLI Profiles, Keyboard Shortcuts; Notifications là reset-only. Các type/domain public đã mô tả cho Notes/Events có thể giữ literal/count tương thích (`None` trong backup counts, `0` trong reset), nhưng không tạo owner, migration, parser v2/v3 hoặc placeholder participant tương lai. `SettingsBackupSection` tái sử dụng public type BE-008 hiện có với notification settings vắng mặt.
- `app/data_participants.rs`, `app/data_runtime.rs`, `shared/maintenance.rs` và maintenance API core đã tồn tại. Mở rộng chúng, không thay bằng service giả. Reset adapter phải nối SessionManager/TerminalManager thật và BE-011 pause/resume; không gọi shutdown true-Quit vốn đóng admission vĩnh viễn. Reset và true-Quit phải serialize admission để Storage không đóng giữa apply/publish; Quit chờ maintenance đang chạy kết thúc, reset mới bị từ chối khi Quit đã được nhận.
- Binding generator hiện hành là `src-tauri/tests/export_bindings.rs`, tự ghi output rồi fail một lần khi drift. Giai đoạn 13 mở rộng generator này; không tạo `src-tauri/src/bin/export_bindings.rs`.
- Settings/Keyboard Shortcuts bổ sung public Rust-only `subscribe()` trả `tokio::sync::watch::Receiver<u64>` nếu chưa có; publish maintenance thông báo sau cache swap, không query lại DB. Counter invalidation process-local không phải revision trong backup. Không thêm IPC hoặc frontend integration ngoài generated binding ở bước BE.
- Parser strict phải phát hiện duplicate key ở mọi object trước khi mất thông tin qua map/`serde_json::Value`; các DTO owner có parser IPC permissive được kiểm tra shape strict trong boundary backup, không đổi hành vi IPC cũ. Field của enum public có dữ liệu dùng camelCase, gồm `fileName`, `schemaVersion`; enum secret reference giữ đúng shape owner hiện hành, không đổi format ngầm.
- Import mở một file handle, kiểm tra regular file trên handle và đọc tối đa 128 MiB + 1 byte để phát hiện file tăng kích thước sau metadata. Picker filter không thay validation content; không execute command/env, không mở source content hoặc duyệt directory từ metadata. Confirm dùng package bất biến trong memory, không đọc lại path. Project path validation/canonicalization chỉ qua owner; test dùng resolver giả hoặc folder tạm.
- Export Windows dùng sibling temp `create_new`, sync rồi atomic replace; không dùng delete-destination-then-rename. Nếu cần API Windows, chỉ mở rộng feature `Win32_Storage_FileSystem` của `windows-sys = 0.61.2` đang khóa; không thêm dependency/version mới. Tên UTC dùng clock/SystemTime hiện hành và chuyển đổi lịch thuần, không thêm thư viện ngày giờ chỉ để đặt tên file.
- Service cung cấp seam Rust-only `DataManagementService::with_seams` nhận Storage, shared gate, participant registry, `DataRuntimeControl`, native adapter, clock monotonic/UTC và event sink. App test constructor `configure_with_data_management_for_tests` inject toàn bộ collaborator OS của Data/Projects/CLI/Notifications và runtime; không fallback sang system adapter. Fault hook chỉ dùng trong test ở boundary transaction/file operation, không đi qua IPC.
- Test chỉ dùng temp app-data/database/project, fake credential/command/shell/environment/dialog/opener/clipboard/OS notification, deterministic clock và barrier. Không đọc/xóa credential, profile, app data hoặc project thật; không đổi biến môi trường toàn process để giả profile. Native smoke reset chỉ được chạy trên Windows profile cô lập đã xác minh, không chạy trên profile đang dùng. Thiếu môi trường cô lập được ghi pending.
- Khi apply bắt đầu, task operation sở hữu permit và toàn bộ chuỗi transaction → publish/resume đến hết; caller hủy future hoặc mất window không được bỏ dở chuỗi sau commit. Fingerprint preview so sánh dữ liệu owner ảnh hưởng merge, phát hiện cả thay đổi giữ nguyên count; không dùng availability runtime không ổn định làm fingerprint. Pending cleanup count phải lấy qua public CLI owner metadata/query, không đọc bảng CLI từ coordinator.
